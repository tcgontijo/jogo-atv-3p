...
function alertWifi($txt, $hasTimer, $countTimer, $srcImg, $fontSize) {
    ...
    if (!$hasTimer) {
        $btnPanelInfo = $("<button></button>").text("Novo Jogo").addClass("button").attr("id", "fecha");
        $btnRanking = $("<button></button>").text("Ranking").addClass("button").attr("id", "ranking");
        $btnLogOut = $("<button></button>").text("Sair").addClass("button").attr("id", "logout");
        $($contentPanel).append($btnPanelInfo);
        $($contentPanel).append($btnRanking);
        $($contentPanel).append($btnLogOut);
